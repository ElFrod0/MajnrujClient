...
public class MobScreen extends AbstractScreen {
    ...
    private final Component noPreview = Component.translatable("majnrujclient.options.no-preview");
    private final Component notImplemented = Component.translatable("majnrujclient.options.not-implemented");

    public MobScreen(Screen parent, Mob mob) {
        super(parent);
        this.mob = mob;
        this.subtitle = Component.translatable("majnrujclient.options.seat.title", mob.getType().getDescription());
    }
    ...
    public void init() {
        ...
        final Seat seat = MajnrujClient.instance().getConfig().seats.getSeat(this.mob);
